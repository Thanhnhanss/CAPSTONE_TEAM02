...
namespace VanLangDoctor.Areas.User.Controllers
{
    public class RegisterConsultationController : Controller
    {
        ...
        public ActionResult RegisterConsultation(DANG_KY dANG_KY, HttpPostedFileBase picture, HttpPostedFileBase picture_CTC)
        {
            if (ModelState.IsValid)
            {
                ...
                else ModelState.AddModelError("", "Hình ?nh không ???c tìm th?y");
                ...
                new MailHelper().SendMail(toEmail, "??n ??ng ký m?i", content);
                ...
                new MailHelper().SendMail(dANG_KY.EMAIL, "??n ??ng ký làm t? v?n viên cho VLDoctor", substance);

                ViewBag.Message = "H? s? ?ã ???c g?i. Vui lòng ki?m tra Email ?? bi?t thêm thông tin";
